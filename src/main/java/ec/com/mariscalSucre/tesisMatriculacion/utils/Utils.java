...
public class Utils {
	...
	public static boolean comprobarCedula(String cedula) {
		if (cedula.compareTo("2222222222") == 0 || cedula.compareTo("4444444444") == 0
				|| cedula.compareTo("5555555555") == 0 || cedula.compareTo("7777777777") == 0
				|| cedula.compareTo("9999999999") == 0 || cedula.compareTo("0000000000") == 0)
			return false;

		if (cedula.length() == 10) {
			int s = 0;
			for (int i = 0; i < cedula.length(); i++) {
				int ced = Integer.parseInt(Character.toString(cedula.charAt(i)));
				if (i % 2 == 0) {
					String c1 = String.valueOf(ced * 2);
					s += c1.length() == 2 ? Integer.parseInt(Character.toString(c1.charAt(0)))
							+ Integer.parseInt(Character.toString(c1.charAt(1))) : Integer.parseInt(c1);
				} else
					s += ced;
			}
	...
	public static boolean comprobarEan13(String codigo) {
		if (!validaNumeros(codigo) && codigo.length() == 13)
			return codigo.substring(codigo.length() - 1, codigo.length())
					.compareToIgnoreCase(checksumEan13(codigo)) == 0 ? true : false;
		return false;
	}
	...
	public static String insertarPalabra(int pos, String palabra, String element) {
		...
		for (int i = 0; i < palabra.length(); i++)
			nuevaPalabra += (i == pos) ? palabra.charAt(i) + element : palabra.charAt(i);
		return nuevaPalabra;
	}
